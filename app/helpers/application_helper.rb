module ApplicationHelper
  ...
  def region_options(mission_scope: Mission.active, current_mission: nil)
    regions = (mission_scope.includes(:region) + [current_mission]).
      compact.
      map(&:region).
      uniq.
      sort_by(&:name)
  ...
  # region is selected.
  def mission_options(mission_scope: Mission.active, current_mission: nil)
    missions = (mission_scope.includes(:region, :agency, :project) + [current_mission]).
      compact.
      uniq.
  ...
  def habitat_type_options(current_region_id: nil)
