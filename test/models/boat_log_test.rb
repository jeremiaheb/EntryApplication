...
class BoatLogTest < ActiveSupport::TestCase
  test "factory creates a valid instance" do
    boat_log = FactoryBot.create(:boat_log)
    assert boat_log.valid?
  end

  test "#boatlog_divers returns a list of all the divers in the boat log" do
    rep_logs1 = FactoryBot.create_list(:rep_log, 3)
    station_log1 = FactoryBot.create(:station_log, stn_number: 1, rep_logs: rep_logs1)
    rep_logs2 = FactoryBot.create_list(:rep_log, 2)
    station_log2 = FactoryBot.create(:station_log, stn_number: 2, rep_logs: rep_logs2)
    boat_log = FactoryBot.create(:boat_log, station_logs: [station_log1, station_log2])
