...
when "production"
  if GoogleCloudDetector.running_on_google_cloud?
    Rails.application.config.action_mailer.default_url_options = { host: "ncrmp-data-entry.fisheries.noaa.gov" }
  else
    Rails.application.config.action_mailer.default_url_options = { host: "grunt.sefsc.noaa.gov" }
  end
